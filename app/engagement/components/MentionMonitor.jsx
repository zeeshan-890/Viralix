...
export default function MentionMonitor() {
    ...
    const filteredMentions = mentions.filter(mention => {
        if (filter !== 'all' && mention.type !== filter)
            return false;
        return true;
    });
    return (<div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="space-y-4">
                {filteredMentions.map((mention) => (<div key={mention.id} className={`border rounded-lg p-4 transition-colors ${mention.read ? 'border-gray-200 bg-white' : 'border-blue-200 bg-blue-50'}`}>
                        <div className="flex items-start space-x-4">
                            ...
                            <div className="flex-1">
                                <div className="flex items-center space-x-2 mb-2">
                                    ...
                                    {!mention.read && (<span className="w-2 h-2 bg-blue-500 rounded-full"></span>)}
                                </div>
                                ...
                                <div className="flex space-x-2">
                                    ...
                                    {!mention.read && (<button className="px-3 py-1 bg-gray-100 text-gray-700 text-sm rounded-lg hover:bg-gray-200 transition-colors">
                                            Mark as Read
                                        </button>)}
                                </div>
                            </div>
            ...
            {filteredMentions.length === 0 && (<div className="text-center py-8 text-gray-500">
