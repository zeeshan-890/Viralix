...
export default function CalendarView() {
    ...
    const getPostsForDay = (day: number | null) => {
        ...
        if (day === 15) return scheduledPosts.slice(0, 2)
        if (day === 20) return scheduledPosts.slice(2, 4)
        if (day === 25) return [scheduledPosts[0]]
        return []
    }
            ...
            <div className="p-6">
                ...
                <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden">
                    {getDaysInMonth(currentDate).map((day, index) => (
                        <div
                            key={index}
                            className={`min-h-[120px] bg-white p-2 ${day ? 'hover:bg-gray-50 cursor-pointer' : ''
                                }`}
                        >
                            {day && (
                                <>
