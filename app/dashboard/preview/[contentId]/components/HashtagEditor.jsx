    ...
    return (<div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3 min-h-[40px] p-2 border border-gray-300 rounded-lg">
                    {hashtags.map((tag, index) => (<span key={index} className="inline-flex items-center px-2 py-1 rounded-full text-sm bg-blue-100 text-blue-800">
                            #{tag}
                            <button onClick={() => removeHashtag(tag)} className="ml-1 text-blue-600 hover:text-blue-800">
                                ×
                            </button>
                        </span>))}
                    <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown} placeholder={hashtags.length === 0 ? "Type hashtags..." : ""} className="flex-1 min-w-[100px] outline-none"/>
                </div>
            </div>
            ...
            .slice(0, 6)
            .map((item) => (<button key={item.tag} onClick={() => addHashtag(item.tag)} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors text-sm">
                                <span>#{item.tag}</span>
                                <div className="flex items-center space-x-1">
                                    <div className={`w-2 h-2 rounded-full ${item.popularity > 90 ? 'bg-green-500' :
                item.popularity > 80 ? 'bg-yellow-500' : 'bg-gray-400'}`}></div>
