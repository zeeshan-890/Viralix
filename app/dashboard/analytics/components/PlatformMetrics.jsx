export default function PlatformMetrics() {
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        ].map((group) => (<div key={group.range} className="flex items-center">
                                <div className="w-16 text-sm text-gray-600">{group.range}</div>
                                <div className="flex-1 bg-gray-200 rounded-full h-2 mx-3">
                                    <div className={`${group.color} h-2 rounded-full`} style={{ width: `${group.percentage}%` }}/>
                                </div>
        ...
        ].map((location) => (<div key={location.country} className="flex items-center justify-between">
