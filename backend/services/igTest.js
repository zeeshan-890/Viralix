...
const GRAPH_VERSION = instagramPublish.GRAPH_VERSION;
...
async function getLongLivedToken(shortLivedToken) {
    try {
        ...
        console.warn('[igTest] Long-lived token exchange failed:', e.response?.data?.error?.message || e.message);
    }
...
async function getProfile(igUserId, token) {
    try {
        ...
        console.warn('[igTest] /me profile fetch failed:', e.response?.data?.error?.message || e.message);
    }
    try {
